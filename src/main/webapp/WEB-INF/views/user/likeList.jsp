...
<body>
  ...
  <h2>${UserVO.userNickName }님이 선택한 좋아요 목록</h2>
  
  
  
  <hr>
  <br>
  <a href="../main/user"><input  style="background-color:#4CAF50;" type="button" class="btn btn-outline-info }" style="float: left;" value="뒤로가기"></a>
  <br>
   <table class="table table-hover table-striped table-condensed" style="border: 1px solid;">
    ...
    <tbody>
      <c:forEach var="vo" items="${playList }">
        <tr>
          <td>${vo.playNo }</td>
          <td style="width: 100px; height: 100px;"><img src="../play/display?fileName=${vo.img }"><br><a href="../play/detail?playNo=${vo.playNo }&page=${pageMaker.criteria.page}">${vo.playTitle }</a></td>
          <td>${vo.userNickname }</td>
    ...
    <tbody>
      <c:forEach var="vo" items="${themeList }">
        <tr>
          <td>${vo.themeNo }</td>
          <td style="width: 100px; height: 100px;"><img src="../theme/display?fileName=${vo.img }"><br><a href="../theme/detail?themeNo=${vo.themeNo }&page=${pageMaker.criteria.page}">${vo.themeTitle }</a></td>
          <td>${vo.userNickname }</td>
          <fmt:formatDate value="${vo.pthemeCdate }"
            pattern="yyyy-MM-dd HH:mm:ss"
            var="themeCdate"/>
          <td>${themeCdate }</td>
          <td>${vo.themeViewCnt }</td>
          <td>${vo.themeReplyCnt }</td>
          <td>${vo.themeGoodCnt }</td>
          <td>${vo.themeWishCnt }</td>
        </tr>
      </c:forEach>
    </tbody>
  ...
  <script type="text/javascript">
  	$(document).ready(function(){
  	  $('.pagination li a').click(function(){
  	    ...
  	    frm.attr('action', 'likeList'); // form에 action 속성-값 추가
